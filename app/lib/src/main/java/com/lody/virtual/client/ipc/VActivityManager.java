...
public class VActivityManager {
    ...
    public int startActivity(Intent intent, ActivityInfo info, IBinder resultTo, Bundle options, String resultWho, int requestCode, int userId) {
        try {
            //getService从server获取AppManagerService的binder，返回一个代理实例，binder与serverInterface保存在代理实例中，代理实例通过serverInterface.getIPCMethod(method).callRemote(binder,args)
            //-> binder.transact(code,data,reply,0), 实现调用目标函数
