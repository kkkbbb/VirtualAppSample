...
/* package */ class ActivityStack {
    ...
    int startActivityLocked(int userId, Intent intent, ActivityInfo info, IBinder resultTo, Bundle options,
        ...
        if (reuseTask == null) {
            startActivityInNewTaskLocked(userId, intent, info, options);//创建Activity
        } else {//Activity已启动 reuse
    ...
    private Intent startActivityProcess(int userId, ActivityRecord sourceRecord, Intent intent, ActivityInfo info) {
        ...
        ProcessRecord targetApp = mService.startProcessIfNeedLocked(info.processName, userId, info.packageName);//先创建启动进程
        if (targetApp == null) {
            return null;
        }
